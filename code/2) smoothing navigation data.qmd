...
plot_ly() %>%
  ...
  add_trace(x = x, y = y, z = - positions$DEPTH2, mode = "markers", type = "scatter3d", name = "un-smoothed", marker = list(size = 4)) %>%
  add_trace(x = sx[[2]], y = sy[[2]], z = - positions$DEPTH2, mode = "lines", type = "scatter3d", name = "smoothed") %>%
  layout(title = "Difference of un-smoothed and smoothed ROV transect",
...
#####ADDITION: only for comparison with non-smoothing or in case smoothing is not desired, apply following steps to extarct the raw coordinates
