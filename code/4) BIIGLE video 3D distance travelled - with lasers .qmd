...
```{r}
### ADDITIONAL: for comparison: smoothed =c("xsmoothed","ysmoothed"),in case when smoothing is not desired and raw coordinates should be used instead c("LON2","LAT2") (raw coordinates from navigation file)
...
# USE interpolated depth HERE - renaming interpolated to just depth for simplicity ##### for 2D distance travelled: remove "depth = depth_interpolated" below
...
### ADDITIONAL: for 2D distance travelled: remove "depth2 = depth" and, distance_3d --> distance_2d: distance_2d = # put coordinates into a matrix
           ####matrix(c(X2,Y2,X,Y), ncol = 2, exchange all 3Ds with 2Ds
dpoint %>%
...
```{r}
## ADDITIONAL: for comparison: exchange all 3Ds with 2Ds
...
joint_navigation_annotation %>%
  write_csv(paste0("./Output/dist_calibrated_unsmoothed_3d_navigation_flat3.csv" ) )
